...
function autoBind(instance: object): void {
    ...
    Object.getOwnPropertyNames(proto).forEach(key => {
        ...
        if (key !== 'constructor' && descriptor && typeof descriptor.value === 'function') {
            (instance as Record<string, unknown>)[key] = (instance as Record<string, () => void>)[key].bind(instance)
        }
    })
}
...
class Title {
    ...
    createMesh() {
      ...
      `,
        ...
        this.mesh.position.y = -(this.plane.scale as { y: number }).y * 0.5 - textHeight * 0.5 - 0.05
...
class Media {
    ...
    update(scroll: ScrollState, direction: string) {
        ...
        const planeOffset = (this.plane.scale as { x: number }).x / 2
    ...
    onResize({ screen, viewport }: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
        ...
        this.scale = this.screen.height / 1500
            ; (this.plane.scale as { y: number }).y = (this.viewport.height * (900 * this.scale)) / this.screen.height
            ; (this.plane.scale as { x: number }).x = (this.viewport.width * (700 * this.scale)) / this.screen.width
        this.plane.program.uniforms.uPlaneSizes.value = [(this.plane.scale as { x: number }).x, (this.plane.scale as { y: number }).y]
        this.padding = 2
        this.width = (this.plane.scale as { x: number }).x + this.padding
...
class GalleryApp {
    ...
    constructor(container: HTMLDivElement, {
        items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05
    }: AppOptions = {}) {
    ...
    destroy() {
        ...
        if (this.renderer?.gl?.canvas?.parentNode) {
            this.renderer.gl.canvas.parentNode.removeChild(this.renderer.gl.canvas as Node)
        }
    }
}
